...
class RealisticDecodeDriver:
    ...
    def bench(
        ...
        bgs = bgs or []
        pre_profiler_bgs = [
            bg for bg in bgs if not getattr(bg, "start_after_profiler", False)
        ]
        post_profiler_bgs = [
            bg for bg in bgs if getattr(bg, "start_after_profiler", False)
        ]
        starts = [torch.cuda.Event(enable_timing=True) for _ in range(iters)]
        stops = [torch.cuda.Event(enable_timing=True) for _ in range(iters)]
        torch.cuda.synchronize(self.device)
        for bg in pre_profiler_bgs:
            bg.start()
        torch.cuda.profiler.cudart().cudaProfilerStart()
        for bg in post_profiler_bgs:
            bg.start()
        ...
        try:
            for i in range(iters):
                ...
                for bg in bgs:
                    before_step = getattr(bg, "before_step", None)
            ...
            torch.cuda.nvtx.range_pop()
            torch.cuda.synchronize(self.device)
            post_bg_stats = [bg.stop() for bg in post_profiler_bgs]
            torch.cuda.profiler.cudart().cudaProfilerStop()
            pre_bg_stats = [bg.stop() for bg in pre_profiler_bgs]
            bg_stats = pre_bg_stats + post_bg_stats
