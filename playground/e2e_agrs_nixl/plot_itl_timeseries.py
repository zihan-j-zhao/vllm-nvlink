...
Output: a PNG (default playground/out/aiperf_pd/e2e_agrs_nixl/<latest>/itl_timeseries.png)
with:
...
If --trace-dir is omitted, picks the newest playground/log/e2e_agrs_nixl/*/pd_trace
directory.
"""
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="Directory of role=*.jsonl trace files. Defaults to the newest "
             "playground/log/e2e_agrs_nixl/*/pd_trace.",
    )
    ap.add_argument(
        "--bin-ms",
        type=float,
    ...
    ap.add_argument(
        ...
        type=float,
        default=None,
        help="If set, crop the plotted window to this many seconds from the selected start time.",
    )
    ap.add_argument(
        "--title",
        type=str,
    ...
    if args.trace_dir is None:
        candidates = sorted(glob.glob("playground/log/e2e_agrs_nixl/*/pd_trace"),
                            key=os.path.getmtime)
        if not candidates:
            raise SystemExit(
                "no --trace-dir given and no playground/log/e2e_agrs_nixl/*/pd_trace "
                "found"
            )
    ...
    if args.duration_sec is not None:
        t1 = min(t1, t0 + args.duration_sec)
