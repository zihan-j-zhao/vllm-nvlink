...
# launched by playground/e2e_agrs_nixl/start_server.sh) with AIPerf.
#
# Identical to playground/moe_a2a/run_aiperf.sh apart from:
#   * Default URL points at the NIXL proxy on port 8000.
#   * Default artifact dir lives under playground/out/aiperf_pd/e2e_agrs_nixl/<UTC>.
#
...
#                        override wins.
#   OUT_DIR              Artifact dir             (default: playground/out/aiperf_pd/e2e_agrs_nixl/<UTC>)
...
OUT_DIR="${OUT_DIR:-playground/out/aiperf_pd/e2e_agrs_nixl/$TS}"
mkdir -p "$OUT_DIR"

AIPERF="${AIPERF:-$(command -v aiperf || true)}"
if [[ ! -x "$AIPERF" ]]; then
    echo "error: aiperf not found at $AIPERF" >&2
    echo "       activate the vllm-nvlink env, set AIPERF=/path/to/aiperf, or run build.sh first" >&2
    exit 1
fi
...
# playground/e2e_agrs_nixl/proxy.py (1) forwards the upstream `text/event-stream`
