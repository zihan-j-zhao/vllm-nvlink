...
#   - 1 proxy server (playground/e2e_agrs_nixl/proxy.py, a fork of
...
#   ALL2ALL_BACKEND    MoE EP backend override, e.g. allgather_reducescatter
#                      or nixl_ep. If unset, vLLM's default is used.
#   MOE_BACKEND        --moe-backend value (default: triton; use auto for
#                      FlashInfer/DeepEP backend experiments)
#   ENABLE_DBO         "1" enables vLLM dual batch overlap (default: 0)
#   DBO_DECODE_TOKEN_THRESHOLD   --dbo-decode-token-threshold (default: 32)
#   DBO_PREFILL_TOKEN_THRESHOLD  --dbo-prefill-token-threshold (default: 512)
#   COMPILATION_CONFIG --compilation-config JSON override (default: unset)
#   NVSHMEM_QP_DEPTH   DeepEP/NVSHMEM QP depth (default: 4096 when unset)
#   SERVED_MODEL_NAME  OpenAI model id (default: basename of $MODEL)
#   GPU_MEM_UTIL       --gpu-memory-utilization per worker (default: 0.85)
#   LOG_DIR            Output dir      (default: playground/log/e2e_agrs_nixl/<UTC>)
#   PY                 Python binary   (default: python on PATH)
...
LOG_DIR="${LOG_DIR:-playground/log/e2e_agrs_nixl/$TS}"
mkdir -p "$LOG_DIR"
...
# Plotter: playground/e2e_agrs_nixl/plot_itl_timeseries.py
...
if [[ ! -x "$PY" ]]; then
    ...
    echo "       activate the vllm-nvlink env or set PY=/path/to/python" >&2
    exit 1
fi
export PATH="$(dirname "$PY"):$PATH"

# CUDA 13 wheels place runtime libraries under site-packages/nvidia/cu13/lib.
# DeepEP built in this environment links against those libraries, so make them
# visible to worker subprocesses when present.
CUDA_WHEEL_LIB="$($PY - <<'PY'
from pathlib import Path
import site

for root in site.getsitepackages():
    lib = Path(root) / "nvidia" / "cu13" / "lib"
    if (lib / "libcudart.so.13").exists():
        print(lib)
        break
PY
)"
if [[ -n "$CUDA_WHEEL_LIB" ]]; then
    export LD_LIBRARY_PATH="$CUDA_WHEEL_LIB:${LD_LIBRARY_PATH:-}"
    export LIBRARY_PATH="$CUDA_WHEEL_LIB:${LIBRARY_PATH:-}"
fi
...
if [[ "$ENFORCE_EAGER" =~ ^(1|true|TRUE|yes|YES|on|ON)$ ]]; then
    echo "[start_server] cuda graphs        = DISABLED (--enforce-eager)"
else
...
if [[ -n "$ALL2ALL_BACKEND" ]]; then
    echo "[start_server] all2all backend     = $ALL2ALL_BACKEND"
else
    echo "[start_server] all2all backend     = vLLM default"
fi
echo "[start_server] kv connector       = NixlConnector"
echo "[start_server] moe backend        = $MOE_BACKEND"
echo "[start_server] gpu-memory-util    = $GPU_MEM_UTIL"
echo "[start_server] model phase trace  = ${VLLM_PD_TRACE_MODEL_PHASES:-0}"
if [[ "$ENABLE_DBO" =~ ^(1|true|TRUE|yes|YES|on|ON)$ ]]; then
    echo "[start_server] dbo                = ENABLED (decode threshold=$DBO_DECODE_TOKEN_THRESHOLD, prefill threshold=$DBO_PREFILL_TOKEN_THRESHOLD)"
else
    echo "[start_server] dbo                = DISABLED"
fi
if [[ -n "$PROFILER_CONFIG" ]]; then
    echo "[start_server] profiler config    = $PROFILER_CONFIG"
fi
if [[ -n "$COMPILATION_CONFIG" ]]; then
    echo "[start_server] compilation config = $COMPILATION_CONFIG"
fi
...
if [[ -n "$ALL2ALL_BACKEND" ]]; then
    COMMON_ARGS+=( --all2all-backend "$ALL2ALL_BACKEND" )
fi
if [[ "$ENFORCE_EAGER" =~ ^(1|true|TRUE|yes|YES|on|ON)$ ]]; then
...
if [[ -n "$PROFILER_CONFIG" ]]; then
    COMMON_ARGS+=( --profiler-config "$PROFILER_CONFIG" )
fi
if [[ -n "$COMPILATION_CONFIG" ]]; then
    COMMON_ARGS+=( --compilation-config "$COMPILATION_CONFIG" )
fi
