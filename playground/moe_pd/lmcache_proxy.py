...
# --- moe_pd additions -------------------------------------------------------
# The upstream LMCache proxy ships neither a readiness probe nor a /v1/models
# forward, but the playground tooling (run_nsys_sweep.sh / aiperf
# --wait-for-model-mode models) probes GET /v1/models on the proxy. Forward it
# to the first prefiller so the model list is reachable through the proxy.
@app.get("/healthcheck")
...
@app.get("/v1/models")
async def models():
    resp = await app.state.prefill_clients[0].client.get("/v1/models")
    return Response(
...
async def handle_completions(request: Request):
    ...
    try:
        ...
        org_max_tokens = req_data.get("max_tokens")
        if org_max_tokens is None:
            org_max_tokens = req_data.get("max_completion_tokens", 1)
...
async def handle_chat_completions(request: Request):
    ...
    try:
        ...
        # aiperf / OpenAI chat clients may send only `max_completion_tokens`
        # (not `max_tokens`). Fall back to it so the proxy doesn't KeyError.
        org_max_tokens = req_data.get("max_tokens")
        if org_max_tokens is None:
            org_max_tokens = req_data.get("max_completion_tokens", 1)
        req_data["prompt"] = tokenize_output["tokens"]
