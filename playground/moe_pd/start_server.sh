...
echo "[start_server] profiler kind      = $VLLM_PROFILE_KIND"
if [[ "$VLLM_PROFILE_KIND" == "torch" ]]; then
    ...
    echo "[start_server] cuda profiler      = ENABLED (/start_profile triggers cudaProfilerStart)"
else
    echo "[start_server] vLLM profiler      = DISABLED"
fi
