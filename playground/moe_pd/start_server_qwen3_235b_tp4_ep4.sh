...
# using vLLM's NixlConnector (NIXL/UCX KV transport), with TP=EP=4 on
# each side.
#
...
# each with tensor-parallel-size=4 + --enable-expert-parallel and PP=1.
# This script intentionally does not set --data-parallel-size: each side is
# one TP=4 engine, and MoE layers use expert parallelism across those ranks.
#
# NIXL side channel: because this setup has DP=1 per side, each side has one
# scheduler and therefore one side-channel port. The port-range overlap check
# is intentionally one-port-per-side, unlike the DP=4 script.
#
...
#   PREFILL_SIDE_PORT  NIXL side ch. port for prefill scheduler (default: 5559)
#   DECODE_SIDE_PORT   NIXL side ch. port for decode  scheduler (default: 5570)
...
#   TP_SIZE            Must be 4 for this TP=EP=4 setup (default: 4)
#   EXPERT_PARALLEL    Must be "1" for this TP=EP=4 setup (default: 1)
#   SERVED_MODEL_NAME  OpenAI model id (default: basename of $MODEL)
#   GPU_MEM_UTIL       --gpu-memory-utilization per worker (default: 0.85)
#   LOG_DIR            Output dir      (default: playground/log/moe_pd/qwen3_235b_tp4_ep4/<UTC>)
#   NIXL_FAKE_READ     "1"/"0" make decode skip NIXL READs and immediately
#                      mark receives complete (default: 0; outputs invalid)
#   NIXL_NUM_THREADS   NIXL UCX worker threads per process (default: 1; lower
#                      values reduce mlx5 UAR pressure on RoCE/NVLink hosts)
#   UCX_NET_DEVICES    UCX network device selection (default: all)
...
SCRIPT_TAG="start_server_235b_tp4_ep4"
...
LOG_DIR="${LOG_DIR:-playground/log/moe_pd/qwen3_235b_tp4_ep4/$TS}"
mkdir -p "$LOG_DIR"
...
PREFILL_GPU_COUNT="$(_count_csv "$PREFILL_GPUS")"
DECODE_GPU_COUNT="$(_count_csv "$DECODE_GPUS")"
if (( TP_SIZE != 4 )); then
    echo "error: TP_SIZE must be 4 for the TP=EP=4 setup" >&2
    exit 1
fi
if (( PREFILL_GPU_COUNT != TP_SIZE )) || (( DECODE_GPU_COUNT != TP_SIZE )); then
    echo "error: this Qwen3-235B TP=EP=4 script requires exactly $TP_SIZE GPUs on both sides" >&2
    echo "       PREFILL_GPUS=$PREFILL_GPUS gives $PREFILL_GPU_COUNT GPUs" >&2
    echo "       DECODE_GPUS=$DECODE_GPUS gives $DECODE_GPU_COUNT GPUs" >&2
    exit 1
fi
EXPERT_PARALLEL="${EXPERT_PARALLEL:-1}"
if [[ "$EXPERT_PARALLEL" != "1" ]]; then
    echo "error: EXPERT_PARALLEL must be 1 for the TP=EP=4 setup" >&2
    exit 1
fi
# DP=1 per side, so each side has one scheduler side-channel port.
if (( PREFILL_SIDE_PORT == DECODE_SIDE_PORT )); then
    echo "error: PREFILL_SIDE_PORT and DECODE_SIDE_PORT must differ" >&2
    exit 1
fi
...
echo "[$SCRIPT_TAG] prefill            = GPUs $PREFILL_GPUS (TP=$TP_SIZE, EP=4, DP=1) :: http $PREFILL_PORT, NIXL side port $PREFILL_SIDE_PORT"
echo "[$SCRIPT_TAG] decode             = GPUs $DECODE_GPUS (TP=$TP_SIZE, EP=4, DP=1) :: http $DECODE_PORT, NIXL side port $DECODE_SIDE_PORT"
...
echo "[$SCRIPT_TAG] tensor parallel    = ENABLED (--tensor-parallel-size $TP_SIZE)"
echo "[$SCRIPT_TAG] expert parallel    = ENABLED (--enable-expert-parallel, EP=4 over TP ranks)"
echo "[$SCRIPT_TAG] data parallel      = DISABLED (DP=1; no --data-parallel-size)"
echo "[$SCRIPT_TAG] kv connector       = NixlConnector"
echo "[$SCRIPT_TAG] nixl num threads   = $NIXL_NUM_THREADS"
echo "[$SCRIPT_TAG] ucx net devices    = $UCX_NET_DEVICES"
...
has_startup_failure() {
    ...
    for log in "${SERVER_LOGS[@]}"; do
        [[ -f "$log" ]] || continue
        if grep -Eq \
            'EngineCore failed to start|WorkerProc hit an exception|RuntimeError: Worker failed|Traceback \(most recent call last\)|Buffer overflow when allocating memory|CUDA out of memory|OutOfMemoryError|NIXL_ERR_BACKEND|UCX endpoint create failed|Failed to create UCX worker' \
            "$log"; then
...
KV_CONFIG_PRODUCER='{"kv_connector":"NixlConnector","kv_role":"kv_producer","kv_connector_extra_config":{"num_threads":'"$NIXL_NUM_THREADS"'}}'
if [[ "$NIXL_FAKE_READ" == "1" ]]; then
    KV_CONFIG_CONSUMER='{"kv_connector":"NixlConnector","kv_role":"kv_consumer","kv_connector_extra_config":{"fake_read":true,"num_threads":'"$NIXL_NUM_THREADS"'}}'
else
    KV_CONFIG_CONSUMER='{"kv_connector":"NixlConnector","kv_role":"kv_consumer","kv_connector_extra_config":{"num_threads":'"$NIXL_NUM_THREADS"'}}'
fi
SERVER_LOGS=("$LOG_DIR/prefill.log" "$LOG_DIR/decode.log")

# vLLM serve args shared by prefill and decode. Same knobs as
# playground/moe_pd/start_server.sh, with Qwen3-235B as the default model and
# TP=4 instead of DP=4.
COMMON_ARGS=(
    --model "$MODEL"
